...
let CarComponent = class CarComponent {
    ...
    ngOnInit() {
        this.route.params.subscribe((params) => {
            ...
            this.car = this.CarsService.getById(+params.id);
        });
    }
};
...
let FilterTwoPipe = class FilterTwoPipe {
    transform(cars, search_two = '') {
        if (!search_two.trim()) {
            return cars;
        }
...
let FilterYearPipe = class FilterYearPipe {
    transform(cars, search_year) {
        if (!search_year.trim()) {
            return cars;
        }
        return cars.filter(car => {
